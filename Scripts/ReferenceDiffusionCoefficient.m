...
ylabel('Voltage / V');
title('Voltage vs. Capacity for Different Diffusion Coefficients');
legend(legendEntries, 'Location', 'best'); % Add custom legend entries
grid on;
hold off;


% Plot dashboards for each diffusion coefficient
for i = 1:numel(D0)
    figure(i + 1); % Create a new figure for each dashboard
    sgtitle(sprintf('State plots for D0 = %.0e m^2/s', D0(i))); % Overall title
