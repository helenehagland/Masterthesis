...
charge_end_idxs = find(diff([charge_idx; 0]) == -1); % Store charge end indices

num_cycles = min(jsonstruct.Control.numberOfCycles, 5); % Prevents extra loops
for cycle = 1:num_cycles
    ...
    if length(find(diff([0; charge_idx]) == 1)) >= cycle && length(charge_end_idxs) >= cycle
        ...
        plot(capacity_charge, voltage_charge, '-', 'LineWidth', 2, 'DisplayName', 'Model Charging Cycle ');
    end
    ...
    if length(find(diff([0; discharge_idx]) == 1)) >= cycle && length(find(diff([discharge_idx; 0]) == -1)) >= cycle
        ...
        % Ensure discharge starts at the last voltage of charge cycle
        if cycle <= length(charge_end_idxs)
            charge_end_voltage = voltage(charge_end_idxs(cycle)); % Last voltage of charge cycle
            voltage(discharge_start) = charge_end_voltage; % Force match
        end
        ...
        plot(capacity_discharge, voltage_discharge, '-', 'LineWidth', 2, 'DisplayName', 'Model Discharging Cycle ');
    end
end
